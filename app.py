...
with tab1:
    ...
    with st.expander("📚 理论背景", expanded=True):
        ...
        with col1:
            st.markdown("<div class='stCard'><h4 style='color: #1e88e5;'>规则基线</h4><p>基于句法分析的启发式规则</p></div>", unsafe_allow_html=True)
        with col2:
            st.markdown("<div class='stCard'><h4 style='color: #43a047;'>神经网络</h4><p>BiLSTM-CRF + 受限自注意力机制</p></div>", unsafe_allow_html=True)
        
        # 关键发现
        st.subheader("关键发现")
        st.markdown("<div class='stCard' style='background-color: #e3f2fd;'>85%的边界决策只需考虑当前词前后3-5个词的上下文</div>", unsafe_allow_html=True)
    ...
    try:
        ...
        st.success("✅ 数据加载成功！")
        st.markdown(f"<div class='stCard'>原始文本长度: <strong>{len(tokens)}</strong> 个词</div>", unsafe_allow_html=True)
    ...
    if tokens:
        ...
        if nlp:
            ...
            # 规则3: 遇到动词短语的根节点且其右兄弟包含从句时切分
            for token in doc:
                if token.dep_ == 'ROOT' and token.pos_ == 'VERB':
                    for right_sibling in token.rights:
                        if right_sibling.dep_ in ['advcl', 'ccomp', 'clausal']:
                            if right_sibling.i < len(baseline_boundaries):
                                baseline_boundaries[right_sibling.i] = 'B'
        else:
            st.info("⚠️ spaCy模型未加载，仅使用基本标点切分规则")
            ...
            for i, (token, boundary) in enumerate(zip(tokens, baseline_boundaries)):
                ...
                if boundary == 'B' or i == len(tokens) - 1:
                    ...
                    if boundary == 'B' and current_edu:
                        edu_text = edu_text[:-len(token)] + f"<span style='background-color: #ffcdd2; padding: 2px 4px; border-radius: 4px; font-weight: 600;'>{token}</span>"
        ...
        with col2:
            ...
            for i, (token, boundary) in enumerate(zip(tokens, boundaries)):
                ...
                if boundary == 'B' or i == len(tokens) - 1:
                    ...
                    # 高亮边界词
                    if boundary == 'B' and current_edu:
                        edu_text = edu_text[:-len(token)] + f"<span style='background-color: #c8e6c9; padding: 2px 4px; border-radius: 4px; font-weight: 600;'>{token}</span>"
                    edu_list.append(edu_text)
                    current_edu = []
        ...
        if st.session_state.module1_demo_run:
            with st.spinner("运行演示中..."):
                ...
                neural_result = ["Although the market was volatile during the third quarter ,", "the company's strategic investments in emerging technologies ,", "which were announced last month ,", "have shown promising returns despite economic uncertainties ."]
                ...
                with col1:
                    ...
                    for i, edu in enumerate(baseline_result):
                        st.markdown(f"<div style='background-color: #ffebee; padding: 8px; border-radius: 4px; margin: 4px 0;'>EDU {i+1}: {edu}</div>", unsafe_allow_html=True)
                
                with col2:
                    st.markdown("**神经网络标注结果**")
                    for i, edu in enumerate(neural_result):
                        st.markdown(f"<div style='background-color: #e8f5e9; padding: 8px; border-radius: 4px; margin: 4px 0;'>EDU {i+1}: {edu}</div>", unsafe_allow_html=True)
                
                # 关键发现总结
                st.info("🔍 **关键发现**：神经网络在'although'和'which'引导的从句处检测到更精确的边界，而规则基线在复杂嵌套结构中容易遗漏边界。")
...
with tab2:
    ...
    with st.expander("📚 理论背景", expanded=True):
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.markdown("<div class='stCard' style='background-color: #e3f2fd;'><h4 style='color: #1e88e5;'>TEMPORAL</h4><p>when, after, before, since（时间）</p></div>", unsafe_allow_html=True)
            st.markdown("<div class='stCard' style='background-color: #e8f5e8;'><h4 style='color: #43a047;'>CONTINGENCY</h4><p>because, since, if, unless（因果/条件）</p></div>", unsafe_allow_html=True)
        with col2:
            st.markdown("<div class='stCard' style='background-color: #fff3e0;'><h4 style='color: #fb8c00;'>COMPARISON</h4><p>but, although, however, while（比较/对比）</p></div>", unsafe_allow_html=True)
            st.markdown("<div class='stCard' style='background-color: #f3e5f5;'><h4 style='color: #7b1fa2;'>EXPANSION</h4><p>and, or, also, for example（扩展/补充）</p></div>", unsafe_allow_html=True)
        
        st.subheader("连接词消歧挑战")
        st.markdown("<div class='stCard' style='background-color: #ffebee;'>同一连接词在不同语境下可能表达不同语义</div>", unsafe_allow_html=True)
    ...
    if detected_connectives:
        ...
        if detected_connectives:
            ...
            # 生成高亮文本
            highlighted_text = user_input.replace(connective, f"<span style='background-color: {color_map[category]}; color: white; padding: 2px 6px; border-radius: 4px; font-weight: 600;'>{connective}</span> <span style='font-size: 0.8em; color: {color_map[category]}; font-weight: 600;'>[{category}]</span>")
            ...
            # 显示论据
            st.subheader("🎯 论据提取")
            st.markdown(f"<div class='stCard' style='background-color: #fff9c4;'><strong>Arg1:</strong> {arg1}</div>", unsafe_allow_html=True)
            st.markdown(f"<div class='stCard' style='background-color: #e8f5e8;'><strong>Arg2:</strong> {arg2}</div>", unsafe_allow_html=True)
            ...
            st.markdown(f"<div class='stCard' style='background-color: {color_map[category]}20;'>{category_descriptions[category]}</div>", unsafe_allow_html=True)
    else:
    ...
    with st.expander("🔍 观察与思考"):
            ...
            with st.spinner("分析中..."):
                ...
                for sentence in sentences:
                    ...
                    results.append({
                        ...
                        '正确类别': 'TEMPORAL' if sentences.index(sentence) == 0 else 'CONTINGENCY'
                    })
                
                # 显示结果表格
                st.subheader("📊 消歧演示结果")
                demo_df = pd.DataFrame(results)
                st.dataframe(demo_df.style.apply(lambda x: ['background-color: #e8f5e9' if x['预测类别'] == x['正确类别'] else 'background-color: #ffebee' for i in x], axis=1))
        ...
        with st.expander("💡 [答案] 'since'的消歧难点和工程解决方案？"):
            st.info('''
            **答案**：在第一个句子中，'since'表达TEMPORAL（时间）关系，描述地理位置的持续状态；在第二个句子中，'since'表达CONTINGENCY（因果）关系，描述毕业时间与居住时间的因果关系。消歧核心难点在于需要理解上下文语义，而非仅依靠词法特征。
            ...
            **PDTB框架价值**：
            ...
            - TEMPORAL.ASYNCHRONOUS.BEFORE：时间先后关系
            - CONTINGENCY.CAUSE.RESULT：因果结果关系
...
with tab3:
    ...
    with st.expander("📚 理论背景", expanded=True):
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.markdown("<div class='stCard' style='background-color: #e3f2fd;'><h4 style='color: #1e88e5;'>实体 vs 表述</h4><p>实体是真实世界中的对象，表述是文本中的指称</p></div>", unsafe_allow_html=True)
            st.markdown("<div class='stCard' style='background-color: #e8f5e8;'><h4 style='color: #43a047;'>共指（Coreference）</h4><p>指向同一实体</p></div>", unsafe_allow_html=True)
        with col2:
            st.markdown("<div class='stCard' style='background-color: #fff3e0;'><h4 style='color: #fb8c00;'>回指（Anaphora）</h4><p>指向先行表述</p></div>", unsafe_allow_html=True)
            st.markdown("<div class='stCard' style='background-color: #f3e5f5;'><h4 style='color: #7b1fa2;'>照应词与先行词</h4><p>照应词指向先行词</p></div>", unsafe_allow_html=True)
    ...
    def mock_coreference_resolution(text):
        ...
        # 处理维基百科示例
            ...
            clusters.append(["SpaceX", "the founder, CEO, and chief engineer of SpaceX"])
            clusters.append(["Tesla, Inc.", "the CEO and product architect of Tesla, Inc."])
        # 处理新闻示例
        ...
        # 通用处理
            ...
            if nlp:
                ...
                if people:
                    # 寻找对应的代词
                    pronouns = [token.text for token in doc if token.pos_ == "PRON" and token.text.lower() in ["he", "she", "his", "her", "they", "their"]]
                    if pronouns:
                ...
                for word in words:
                    if word.istitle() and word[0].isalpha():
                        ...
                        if len(current_person) >= 2:
                            people.append(' '.join(current_person))
                        current_person = []
                if len(current_person) >= 2:
                    people.append(' '.join(current_person))
                ...
                pronouns = []
                for word in words:
                    if word.lower() in ["he", "she", "his", "her", "they", "their"]:
                        pronouns.append(word)
                
                if people and pronouns:
                    clusters.append(people + pronouns)
        ...
        for i, cluster in enumerate(clusters):
            ...
            for mention in cluster:
                if mention in highlighted_text:
                    highlighted_text = highlighted_text.replace(mention, f"<span style='background-color: {color}; padding: 4px 6px; border-radius: 6px; font-weight: 600;'>{mention} [Cluster {i+1}]</span>")
    ...
    with st.expander("🔍 观察与思考"):
        ...
        def perform_coreference_resolution(text):
            # 调用现有的mock_coreference_resolution函数
        ...
        if st.session_state.module3_demo_run:
            with st.spinner("运行演示中..."):
                ...
                # 调用指代消解函数
                demo_clusters = perform_coreference_resolution(demo_text)
                ...
                for i, cluster in enumerate(demo_clusters):
                    ...
                    for mention in cluster:
                        if mention in highlighted_text:
                            highlighted_text = highlighted_text.replace(mention, f"<span style='background-color: {color}; padding: 2px 4px; border-radius: 3px;'>{mention}</span>")
                st.markdown(highlighted_text, unsafe_allow_html=True)
                
                # 关键发现总结
                st.info("🔍 **关键发现**：模型成功识别了跨句子的回指关系，包括'Barack Obama'与'He'、'The president'的共指，以及'Hillary Rodham Clinton'与'she'的共指。")
